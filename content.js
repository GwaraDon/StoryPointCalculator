...
(function() {
    ...
    storyPointElements.forEach((element, index) => {
        ...
        // Skip "None" or empty values, only process numeric story points
        if (text && text !== 'None' && !isNaN(parseFloat(text))) {
            const points = parseFloat(text);
            total += points;
            console.log(`Element ${index}: Found value ${points}`);
        } else {
            console.log(`Element ${index}: Skipped (value: "${text}")`);
        }
    });

    console.log(`Calculated Total: ${total}`);

    // Send the final total back to the popup.js
    chrome.runtime.sendMessage({ action: "sendTotal", total: total });
